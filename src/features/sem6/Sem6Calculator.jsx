...
import {
    sem6SubjectsCGPA_CSECore, sem6SubjectsCGPA_AIML, sem6SubjectsCGPA_CD, sem6SubjectsCGPA_CY, sem6SubjectsCGPA_ISE,
    sem6SubjectsCGPA_ECE, sem6SubjectsCGPA_ETE, sem6SubjectsCGPA_EEE, sem6SubjectsCGPA_EIE
} from '../../data/subjectsSem6';
...
const Sem6Calculator = ({ onBack, currentBranch: initialBranch }) => {
    // Navigation State
    const [currentMode, setCurrentMode] = useState(() => loadFromStorage(STORAGE_KEYS.CURRENT_MODE + '_sem6', ''));
    const [branch, setBranch] = useState(() => initialBranch || loadFromStorage(STORAGE_KEYS.CURRENT_BRANCH + '_sem6', ''));

    // Calculation State
    const [subjectGrades, setSubjectGrades] = useState(() => loadFromStorage(STORAGE_KEYS.SUBJECT_GRADES_SEM6, {}));
    const [formData, setFormData] = useState(() => loadFromStorage(STORAGE_KEYS.FORM_DATA_SEM6, {}));
    const [sem6Grades, setSem6Grades] = useState(() => loadFromStorage(STORAGE_KEYS.SEM6_GRADES, {}));
    const [prevSemCGPA, setPrevSemCGPA] = useState(() => loadFromStorage(STORAGE_KEYS.FIRST_YEAR_CGPA + '_sem6', ''));
    ...
    // Persist
    useEffect(() => {
        saveToStorage(STORAGE_KEYS.CURRENT_MODE + '_sem6', currentMode);
    }, [currentMode]);

    useEffect(() => {
        saveToStorage(STORAGE_KEYS.CURRENT_BRANCH + '_sem6', branch);
    }, [branch]);

    useEffect(() => {
        saveToStorage(STORAGE_KEYS.SUBJECT_GRADES_SEM6, subjectGrades);
    }, [subjectGrades]);

    useEffect(() => {
        saveToStorage(STORAGE_KEYS.FORM_DATA_SEM6, formData);
    }, [formData]);

    useEffect(() => {
        saveToStorage(STORAGE_KEYS.SEM6_GRADES, sem6Grades);
    }, [sem6Grades]);

    useEffect(() => {
        saveToStorage(STORAGE_KEYS.FIRST_YEAR_CGPA + '_sem6', prevSemCGPA);
    }, [prevSemCGPA]);

    // Handlers
    const getSubjects = useCallback(() => {
        switch (branch) {
            case 'cse-aiml': return sem6SubjectsCGPA_AIML;
            case 'cd': return sem6SubjectsCGPA_CD;
            case 'cy': return sem6SubjectsCGPA_CY;
            case 'ise': return sem6SubjectsCGPA_ISE;
            case 'ece': return sem6SubjectsCGPA_ECE;
            case 'ete': return sem6SubjectsCGPA_ETE;
            case 'eee': return sem6SubjectsCGPA_EEE;
            case 'eie': return sem6SubjectsCGPA_EIE;
            case 'cse-core':
            default: return sem6SubjectsCGPA_CSECore;
        }
    }, [branch]);
    ...
    const currentSubjects = useMemo(() => {
        ...
        if (currentMode === 'cie-final') {
            return subjects.filter(s => s.type !== 'project');
        }
        return subjects;
    ...
    const handleCalculateSubject = useCallback((subject) => {
        ...
        if (currentMode === 'cie-final') {
            ...
            } else if (subject.type === '50-mark' || subject.type === 'project') {
    ...
    const handleComputeSGPA = () => {
        ...
        const missingSubjects = currentSubjects.filter(s => sem6Grades[s.id] === undefined || sem6Grades[s.id] === '');
        ...
        currentSubjects.forEach(subject => {
            const grade = sem6Grades[subject.id];
        ...
        setSgpaPopup({ isOpen: true, sgpa: roundedSGPA, cycleName: '6th Sem SGPA' });
    };

    const handleComputeCGPA = () => {
        // Validate prev sem CGPA is entered
        if (!prevSemCGPA || parseFloat(prevSemCGPA) <= 0) {
            setValidationError(prev => ({ ...prev, cgpa: 'Please enter CGPA up to 5th Sem first' }));
            return;
        }

        // Validate all 6th sem subjects have grades
        const missingSubjects = currentSubjects.filter(s => sem6Grades[s.id] === undefined || sem6Grades[s.id] === '');
        if (missingSubjects.length > 0) {
            setValidationError(prev => ({ ...prev, cgpa: 'Please enter grades for all 6th Sem subjects' }));
            return;
        }
        ...
        // Up to 5th Semester: 106 Credits total
        if (prevSemCGPA && parseFloat(prevSemCGPA) > 0) {
            totalGradePoints += parseFloat(prevSemCGPA) * 106;
            totalCredit += 106;
        }
        ...
        return (
            <div className="space-y-8 animate-fade-in">
                <div className="flex items-center justify-between mb-8">
                    <button
                        onClick={onBack}
                        className="text-blue-600 hover:text-blue-700 font-medium text-lg transition-colors flex items-center gap-1"
                    >
                ...
                <div className="max-w-md mx-auto bg-white border border-gray-200 rounded-3xl p-8 shadow-sm">
                    ...
                    <div className="grid gap-4">
                        ...
                        <div className="border border-gray-200 rounded-2xl overflow-hidden">
                            ...
                            <div
                                className={`transition-all duration-300 ease-in-out ${expandedClusters.cs ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'
                                    } overflow-hidden`}
                            >
                                <div className="p-4 space-y-3 bg-white">
                                    {[
                                        ...
                                        { id: 'cd', name: 'CSE (Data Science)' },
                                        { id: 'cy', name: 'CSE (Cyber Security)' },
                                        { id: 'ise', name: 'ISE' }
                                    ].map(b => (
                                        <button
                                            key={b.id}
                        ...
                        <div className="border border-gray-200 rounded-2xl overflow-hidden">
                            ...
                            <div
                                className={`transition-all duration-300 ease-in-out ${expandedClusters.ece ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'
                                    } overflow-hidden`}
                            >
                                <div className="p-4 space-y-3 bg-white">
                                    {[
                ...
                <div className="max-w-2xl mx-auto">
                    <div className="bg-white border border-gray-200 rounded-3xl p-8 shadow-sm">
                        <div className="grid grid-cols-[1fr_auto] gap-3 items-start mb-6">
                            <h3 className="text-xl sm:text-2xl font-bold text-gray-900 whitespace-nowrap flex items-center gap-2">
                                ...
                                6th Sem SGPA ({branch.toUpperCase()})
                            </h3>
                        ...
                        <div className="space-y-3">
                            {currentSubjects.map((subject) => (
                                <SubjectCard
                                    key={subject.id}
                                    subject={subject}
                                    gradeValue={sem6Grades[subject.id]}
                    ...
                    <div className="bg-white border border-gray-200 rounded-3xl p-8 shadow-sm mt-8">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-xl font-bold text-gray-900">CGPA up to 5th Sem</h3>
                            <span className="inline-block bg-purple-100 text-purple-800 text-sm font-medium px-3 py-1 rounded-full">
                                106 Credits
                            </span>
                        </div>
                        <div className="bg-purple-50 rounded-xl p-4 border border-purple-200">
                            <label className="text-purple-900 font-medium block mb-2">Enter your CGPA up to 5th Semester</label>
                            <input
                                type="text"
                                inputMode="decimal"
                                value={prevSemCGPA}
                    ...
                    {!showResetConfirm ? (
                        ...
                        <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-xl">
                            <p className="text-sm text-red-700 mb-3">Are you sure you want to reset all grades and CGPA up to 5th Sem?</p>
    ...
    return (
        <div className="space-y-6 sm:space-y-8 animate-fade-in">
            <div className="px-4">
                <button
                    onClick={() => setCurrentMode('')}
                    className="text-blue-600 hover:text-blue-700 font-medium text-base sm:text-lg transition-colors mb-4 flex items-center gap-1"
                >
                    <span>←</span> Back to Modes
                </button>
                <div className="text-center">
                    <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">
                        Sem 6 ({branch.toUpperCase()})
                    </h2>
